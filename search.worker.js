// Search Web Worker — keep scoring logic in sync with app.js
...
function getMatchScore(item, tokens) {
  ...
  for (const token of tokens) {
    if (item.commandLower.startsWith(token)) { score += 12; continue; }
    if (item.commandLower.includes(token)) { score += 8; continue; }
    if (item.tagsLower.some((tag) => tag === token)) { score += 7; continue; }
    if (item.categoryLower.includes(token)) { score += 5; continue; }
    if (item.searchBlob.includes(token)) { score += 3; continue; }

    if (token.length >= 3) {
      if (fuzzyScore(token, item.commandLower) >= 0) { score += 2; continue; }
      if (fuzzyScore(token, item.descriptionLower) >= 0) { score += 1; continue; }
    }
